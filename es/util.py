...
def bulk_error_process(items):
    ...
    return [value for item in items for value in item.values if _.has(value, 'error')]
...
def list_projection(data, projection):
    ...
    #
    # return result
    return [dict_projection(d, projection) for d in data]
...
def values(obj, mapping=None):
    if mapping and isinstance(mapping, list):
        ...
        # return r
        return [_.get(obj, m) for m in mapping]
    else:
        return _.values(obj)
...
def map_(_list, mapping=None, add_title=False):
    if mapping and isinstance(mapping, list):
        def gen_item(_o, _m):
            d = _.get(_o, _.get(_m, 'key'))
            return d if d is not None else _.get(_m, 'default')
        ...
        # r.append(item)
        r = [[gen_item(obj, m) for m in mapping] for obj in _list]
        ...
        if add_title:
            ...
            # r.append(title)
            r.insert(0, [_.get(m, 'title') for m in mapping])
        return r
    else:
        return _.map_(_list)
